...
--
-- Host: 127.0.0.1
-- Tempo de geração: 14-Abr-2022 às 16:42
...
-- Procedimentos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `ValoresAreceberIntervalo` (IN `data_inicial` DATE, IN `data_final` DATE)  BEGIN

Declare acabou Boolean;
Declare nome_cliente varchar(40);
Declare soma_valores_receber float;
DECLARE nmCursor CURSOR FOR
SELECT cliente.nome, sum(valor_receber)
from cliente, contrato, ContasAReceber
where cpf = cliente_cpf and numero = contrato_numero and data_vencimento BETWEEN data_inicial AND data_final and valor_recebido is null
group by cliente.nome;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET acabou = true;
OPEN nmCursor;
drop table if exists auxiliar;
create table auxiliar (nome varchar(40), valor_receber float);
SET acabou = false;
set soma_valores_receber = 0;
WHILE (not acabou) DO
FETCH nmCursor INTO nome_cliente, soma_valores_receber;
if (not acabou) then
insert into auxiliar values (nome_cliente, soma_valores_receber);
end if;
END WHILE;
CLOSE nmCursor;

#para executar 
# CALL ValoresAreceberIntervalo('2021-07-19','2021-11-19');

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ValorRecebidoCli` (IN `cpfCliente` VARCHAR(14), OUT `TotalValorRecebido` DOUBLE)  begin

 select sum(valor_recebido) into TotalValorRecebido
 from contrato, ContasAReceber
 where cliente_cpf = cpfCliente and numero = contrato_numero;

#para executar
#CALL ValorRecebidoCli('234.567.890-10',@total);
#SELECT @total;
...
(1, '2021-08-20', 150, '2021-08-20', 150, 2),
...
(2, 2, '264.557.323-00', 150, '2021-08-20', 1),
...
ALTER TABLE `contasareceber`
  ...
  ADD KEY `Indice_Data_Vencimento` (`data_vencimento`);
...
--
ALTER TABLE `contasareceber`
  ADD CONSTRAINT `fk_ContasAReceber_contrato1` FOREIGN KEY (`contrato_numero`) REFERENCES `contrato` (`numero`) ON DELETE CASCADE;

--
-- Limitadores para a tabela `contrato`
--
ALTER TABLE `contrato`
  ADD CONSTRAINT `fk_pacote_has_cliente_cliente1` FOREIGN KEY (`cliente_cpf`) REFERENCES `cliente` (`cpf`),
  ADD CONSTRAINT `fk_pacote_has_cliente_pacote1` FOREIGN KEY (`pacote_codigo`) REFERENCES `pacote` (`codigo`);

--
-- Limitadores para a tabela `passeio`
--
ALTER TABLE `passeio`
  ADD CONSTRAINT `fk_passeio_guia1` FOREIGN KEY (`guia_codigo`) REFERENCES `guia` (`codigo`);

--
-- Limitadores para a tabela `passeiosdopacote`
--
ALTER TABLE `passeiosdopacote`
  ADD CONSTRAINT `fk_pacote_has_passeio_pacote` FOREIGN KEY (`pacote_codigo`) REFERENCES `pacote` (`codigo`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_pacote_has_passeio_passeio1` FOREIGN KEY (`passeio_codigo`) REFERENCES `passeio` (`codigo`) ON DELETE CASCADE;
COMMIT;
